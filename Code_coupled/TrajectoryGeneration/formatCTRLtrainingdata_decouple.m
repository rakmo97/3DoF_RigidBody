...
for i = 1:length(datafiles)
    ...
    for j = 1:lastidx
        ...
        for k = 1:100
            ...
            Xfull_2(count,:) = [d.stateOut(end,2,j)-d.stateOut(k,2,j),...
                d.stateOut(end,3,j)-d.stateOut(k,3,j),...
                d.stateOut(end,4,j)-d.stateOut(k,4,j),...
                d.stateOut(end,5,j)-d.stateOut(k,5,j),...
                d.stateOut(end,6,j)-d.stateOut(k,6,j),...
                d.stateOut(end,7,j)-d.stateOut(k,7,j),...
                d.stateOut(k,8,j),...
                ];
            ...
            tfull_2(count,:) = [(D*[d.ctrlOut(k,1,j),d.ctrlOut(k,2,j)]')',d.ctrlOut(k,1,j)];
...
save('../NetworkTraining/ANN2_decoupled_data.mat','Xfull_2','tfull_2','Xtrain2','ttrain2','Xtest2','ttest2','times_train','times_test');
