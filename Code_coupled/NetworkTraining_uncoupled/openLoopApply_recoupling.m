...
Fi = zeros (n_times, nCtrlc);
r = 1;



ctrlProfile = zeros(100,3);
for i = 1:100
    phi = trajFromOCL(i,3);
    D = [cos(phi),-sin(phi);sin(phi),cos(phi);1,0];
    ctrlProfile(i,:) = D*ctrlProfileOrig(i,:)';
%     Fi(i,:) = (D'*D)\D' * ctrlProfile(i,:)';
end
...
%     Fi(i,:) = inv(D'*D)*D' * (TxTyMsave(i,:))';
    Fi(i,:) = (D'*D) \ D' * (ctrlProfile(i,:))';

    u1 = Fi(i,1); u2 = Fi(i,2);
    options = odeset('RelTol',1e-200);
    [tsol, xsol] = ode113(@(t,x) dynamics_coupled(t,x,u1,u2), [times(i),times(i+1)], x(i,:),options);
    x(i+1,:) = xsol(end,:);
